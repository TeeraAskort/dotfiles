...
options cryptdevice=/dev/disk/by-uuid/${3}:luks:allow-discards root=/dev/lvm/root resume=UUID=$(blkid -s UUID -o value /dev/lvm/swap) intel_idle.max_cstate=1 apparmor=1 lsm=lockdown,yama,apparmor intel_iommu=igfx_off splash rd.udev.log_priority=3 vt.global_cursor_default=0 rw
EOF
...
options cryptdevice=/dev/disk/by-uuid/${3}:luks:allow-discards root=/dev/lvm/root resume=UUID=$(blkid -s UUID -o value $3) intel_idle.max_cstate=1 apparmor=1 lsm=lockdown,yama,apparmor intel_iommu=igfx_off splash rd.udev.log_priority=3 vt.global_cursor_default=0 rw
EOF
bootctl update
