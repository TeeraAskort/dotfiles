...
if [ "$1" == "gnome" ] || [ "$1" == "kde" ] || [ "$1" == "plasma" ] || [ "$1" == "xfce" ]; then
	...
	# Installing drivers
	apt install -y libgl1-mesa-dri libglx-mesa0 mesa-vulkan-drivers xserver-xorg-video-all libglx-mesa0:i386 mesa-vulkan-drivers:i386 libgl1-mesa-dri:i386 firmware-linux-nonfree
	...
	if [ "$1" == "gnome" ]; then
		...
		# Installing required packages
		apt install -y qbittorrent palapeli kmahjongg kpat thunderbird yakuake gnome-keyring libpam-gnome-keyring libpam-kwallet5 sddm-theme-breeze
